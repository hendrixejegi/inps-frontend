...
const navigation = [
  ...
  { label: "Bills", href: "/bursary/bills", icon: FileText },
  ...
  { label: "Settings", href: "/bursary/settings", icon: Settings },
];

interface BursarySidebarProps {
  collapsed?: boolean;
  onCollapse?: () => void;
  onNavigate?: () => void;
}

export function BursarySidebar({ collapsed = false, onCollapse, onNavigate }: BursarySidebarProps) {
  return (
      ...
      <div className="flex h-20 items-center gap-3 border-b border-white/10 px-5">
        ...
        {!collapsed && (
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-white/60">Bursary & Finance</p>
          </div>
        )}
      </div>

      <nav aria-label="Bursary navigation" className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
