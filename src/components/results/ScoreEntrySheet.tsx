...
export default function ScoreEntrySheet({
  ...
  const calculateTotal = (ca1: number | null, ca2: number | null, exam: number | null) => {
  ...
  const getStatusIcon = (isComplete: boolean) => {
    ...
    const hasAnyScore = students.some(s => s.ca1Score !== null || s.ca2Score !== null || s.examScore !== null);
    if (hasAnyScore) {
      ...
      <table className="w-full">
        <thead className="bg-muted">
          <tr>
            ...
            {showRemarks && <th className="text-left p-3 font-medium">Remarks</th>}
          </tr>
        </thead>
        <tbody>
