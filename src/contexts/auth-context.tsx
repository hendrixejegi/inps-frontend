...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string, type: UserType) => {
    setIsLoading(true);
    try {
      const endpoint = type === "staff" ? "/api/staff/login" : "/api/parent/login";
      const response = await apiClient.post<LoginResponse>(endpoint, { email, password });

      if (response.success && response.token && response.user) {
        localStorage.setItem("auth_token", response.token);
        if (response.refreshToken) localStorage.setItem("refresh_token", response.refreshToken);
        localStorage.setItem("user_data", JSON.stringify({ user: response.user, userType: type }));
  ...
  const refreshToken = useCallback(async () => {
    ...
    const currentType = localStorage.getItem("user_data") ? JSON.parse(localStorage.getItem("user_data") || "{}").userType : null;
    
    if (refreshToken) {
      try {
        // Use appropriate refresh endpoint based on user type
        const endpoint = currentType === "parent" ? "/api/parent/refresh-token" : "/api/staff/refresh-token";
        const response = await apiClient.post<{ token: string; refreshToken?: string }>(endpoint, { refreshToken });
        if (response.token) {
          localStorage.setItem("auth_token", response.token);
          if (response.refreshToken) localStorage.setItem("refresh_token", response.refreshToken);
        }
  ...
  const hasRole = useCallback((roles: StaffRole[]) => {
