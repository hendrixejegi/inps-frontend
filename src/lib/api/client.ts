...
client.interceptors.response.use(
  (response) => {
    ...
    console.log(`✅ [API Client] ${response.config.method?.toUpperCase()} Response:`, {
  ...
  async (error: AxiosError<ApiErrorBody>) => {
    ...
    console.error(`❌ [API Client] ${error.config?.method?.toUpperCase()} Error:`, {
    ...
    if (error.response?.status === 401 && request && !request._retry) {
      ...
      if (refreshToken) {
        ...
        try {
          ...
          const endpoint = userType === "parent" ? "/api/parent/refresh-token" : "/api/staff/refresh-token";
    ...
    return Promise.reject(new ApiError(error.response?.data?.message ?? fallback, status, error.response?.data?.errors));
  },
);
...
export const apiClient = {
  get: <T>(url: string, params?: unknown) => {
    ...
    return client.get<T>(url, { params }).then((response) => {
      console.log(`📡 [API Client] GET request completed:`, { url, status: response.status });
  ...
  post: <T>(url: string, data?: unknown) => {
    ...
    return client.post<T>(url, data).then((response) => {
      console.log(`📡 [API Client] POST request completed:`, { url, status: response.status });
      return response.data;
    });
  },
  put: <T>(url: string, data?: unknown) => {
    ...
    return client.put<T>(url, data).then((response) => {
      console.log(`📡 [API Client] PUT request completed:`, { url, status: response.status });
      return response.data;
    });
  },
  patch: <T>(url: string, data?: unknown) => {
    ...
    return client.patch<T>(url, data).then((response) => {
      console.log(`📡 [API Client] PATCH request completed:`, { url, status: response.status });
      return response.data;
    });
  },
  delete: <T>(url: string, data?: unknown) => {
    ...
    return client.delete<T>(url, { data }).then((response) => {
      console.log(`📡 [API Client] DELETE request completed:`, { url, status: response.status });
      return response.data;
    });
  },
  upload: <T>(url: string, formData: FormData, onProgress?: (percentage: number) => void) => {
    ...
    return client.post<T>(url, formData, {
      ...
      console.log(`📡 [API Client] UPLOAD request completed:`, { url, status: response.status });
      return response.data;
    });
  },
};
