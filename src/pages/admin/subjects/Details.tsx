...
import { ArrowLeft, Loader2, Pencil, Trash2, Power, PowerOff } from "lucide-react";
...
export default function SubjectDetails() {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => adminApi.deleteSubject(subjectId!),
    onSuccess: () => {
  ...
  const toggleActiveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(`Subject ${!subjectData?.data?.isActive ? "activated" : "deactivated"} successfully`);
      refetch();
    },
    onError: (error: Error) => {
      ...
      <div className="mx-auto max-w-[1500px] space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Subject Details</h1>
              <p className="text-sm text-muted-foreground">View and manage subject information</p>
            </div>
          </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold uppercase">{subject.subjectCode}</p>
              </div>
              {subject.description && (
                <div>
              ...
              <div className="flex gap-2">
                ...
                {subject.status && (
                  <div>
                    ...
                    <Badge variant={subject.status === SubjectStatus.ACTIVE ? "default" : "secondary"}>
                      {subject.status}
                    </Badge>
                  </div>
                )}
          ...
          <Card>
            ...
            <CardContent>
              {levels.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">No levels assigned</p>
              )}
            </CardContent>
          </Card>
