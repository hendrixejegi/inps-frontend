...
export default function ClassSubjects() {
  ...
  const { data: assignedSubjects, isLoading: assignedLoading, refetch } = useQuery({
  ...
  if (assignedSubjects?.data && selectedSubjectIds.length === 0) {
    const ids = assignedSubjects.data.map((item: any) =>
      item.subject?.id || item.subjectId
    ).filter(Boolean);
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Successfully assigned ${data.data.added} subjects to class`);
      refetch();
    },
    onError: (error: Error) => {
  ...
  const removeMutation = useMutation({
    mutationFn: (subjectId: string) =>
      adminApi.removeSubjectFromClass(classId!, subjectId, sessionData!.data!.id),
    onSuccess: () => {
  ...
  const handleSave = () => {
    ...
    assignMutation.mutate(selectedSubjectIds);
  };

  const availableSubjects = allSubjects?.data?.filter((subject: any) => {
      ...
      <AdminLayout>
        <div className="mx-auto max-w-[1500px] space-y-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Manage Class Subjects</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              {cls.className || cls.name} - {currentTerm?.term || currentTerm?.session || "Current Term"}
            </p>
          </div>
          <Button 
            onClick={handleSave} 
            ...
            {selectedSubjectIds.length === 0 ? (
              ...
              <div className="space-y-2">
                {selectedSubjectIds.map((subjectId) => {
                  const subject = allSubjects?.data?.find((s: any) => s.id === subjectId);
