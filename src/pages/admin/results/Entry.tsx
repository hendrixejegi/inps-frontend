...
import ScoreValidation, { validateScores } from "@/components/results/ScoreValidation";
...
export default function ResultsEntry() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      try {
        ...
        console.log("🎯 [Results Entry] Current term response:", currentTermResponse);
        if (currentTermResponse && currentTermResponse.data) {
          console.log("✅ [Results Entry] Current term loaded successfully:", currentTermResponse.data);
          ...
          const allTerms = [
            currentTermData,
            { id: 'placeholder-second', term: 'SECOND_TERM', status: 'UPCOMING' },
      ...
      try {
        ...
        if (sessionsResponse && sessionsResponse.data) {
          console.log("✅ [Results Entry] Sessions loaded successfully:", sessionsResponse.data);
          setAcademicYears(sessionsResponse.data);
          if (sessionsResponse.data.length > 0) {
            console.log("📌 [Results Entry] Setting default academic year:", sessionsResponse.data[0].id);
            setAcademicYear(sessionsResponse.data[0].id);
          }
        } else if (Array.isArray(sessionsResponse)) {
          console.log("✅ [Results Entry] Sessions loaded as array:", sessionsResponse);
          // Handle case where response is directly an array
          setAcademicYears(sessionsResponse);
          if (sessionsResponse.length > 0) {
            console.log("📌 [Results Entry] Setting default academic year from array:", sessionsResponse[0].id);
      ...
      if (classesResponse && classesResponse.data) {
        console.log("✅ [Results Entry] Classes loaded successfully:", classesResponse.data);
        setClasses(classesResponse.data);
      } else {
        console.error("❌ [Results Entry] Failed to load classes - unexpected response structure");
      }
  ...
  const loadStudents = async () => {
    if (!academicYear || !termId || !classId) {
      alert("Please select academic year, term, and class");
      return;
    }
    ...
    try {
      ...
      const subjectsResponse = await adminApi.getSubjectsByClass(classId, termId);
      
      let transformedSubjects: any[] = [];
      
      if (subjectsResponse && subjectsResponse.data) {
        // Transform subject data to match expected structure
        transformedSubjects = subjectsResponse.data.map((classSubject: any) => ({
      ...
      const selectedSession = academicYears.find((session: any) => session.id === academicYear);
          ...
          if (existingResultsResponse && existingResultsResponse.data) {
            ...
            const studentsWithResults = studentsData.map((student: any) => {
              ...
              if (existingStudent && existingStudent.subjects) {
                ...
                const subjectsWithResults = transformedSubjects.reduce((acc: any, subject: any) => {
            ...
            const completedCount = existingResultsResponse.data.reduce((count: number, student: any) => {
              return count + Object.values(student.subjects).filter((subject: any) => subject.isComplete).length;
            }, 0);
            
            setCompletedEntries(completedCount);
          }
        } catch (error) {
          console.warn("⚠️ [Results Entry] No existing results found or error loading:", error);
  ...
  const handleScoreChange = (
    ...
    setStudents((prevStudents) =>
      prevStudents.map((student) => {
        ...
        const isComplete = ca1Score !== null && ca2Score !== null && examScore !== null;
  ...
  const handleSubjectNavigation = (direction: "previous" | "next") => {
    if (direction === "previous") {
      setCurrentSubjectStart(Math.max(0, currentSubjectStart - subjectsPerPage));
    } else {
      setCurrentSubjectStart(
        Math.min(currentSubjectStart + subjectsPerPage, subjects.length - subjectsPerPage)
      );
    }
  };
  ...
  const handlePreviousStudent = () => {
    const currentIndex = students.findIndex((s) => s.studentId === selectedStudent);
    if (currentIndex > 0) {
  ...
  const handleSubmitAll = async () => {
    try {
      ...
      const errors: any[] = [];
      students.forEach((student) => {
        Object.entries(student.subjects).forEach(([subjectId, subjectData]: [string, any]) => {
      ...
      for (const [subjectId, scores] of subjectScoresMap.entries()) {
        try {
          ...
          console.error(`Error submitting scores for subject ${subjectId}:`, error);
        }
      }

      if (successCount > 0) {
        alert(`Results submitted successfully for ${successCount} subject(s)!`);
  ...
  const displayedSubjects = subjects.slice(currentSubjectStart, currentSubjectStart + subjectsPerPage);
      ...
      <div className="space-y-6">
        ...
        <ClassSubjectSelector
          ...
          onTermChange={(termValue, termIdValue) => {
            console.log("🔄 [Results Entry] Term changed:", { termValue, termIdValue });
            setTerm(termValue);
            ...
            {entryMode === "subject" && selectedSubject && (
              <SubjectView
                ...
                students={students.map((student) => ({
                  ...
                  examScore: student.subjects[selectedSubject]?.examScore || null,
                  total: student.subjects[selectedSubject]?.total || 0,
                  grade: student.subjects[selectedSubject]?.grade || "F",
                  isComplete: student.subjects[selectedSubject]?.isComplete || false,
                }))}
            ...
            {entryMode === "student" && selectedStudent && (
              <StudentView
                ...
                subjectScores={subjects.map((subject) => ({
                  ...
                  ca1Score: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.ca1Score || null,
                  ca2Score: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.ca2Score || null,
                  examScore: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.examScore || null,
                  total: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.total || 0,
                  grade: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.grade || "F",
                  isComplete: students.find((s) => s.studentId === selectedStudent)?.subjects[subject.id]?.isComplete || false,
                }))}
